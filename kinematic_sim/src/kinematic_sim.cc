...
#include <kinematic_sim/rosmap.hpp>

using namespace std;

////////////////////////////////////////////////////////////////////////////////
geometry_msgs::Quaternion createQuaternionFromYaw(double theta)
{
...
public:
        ...
        double _x=0.0, double _y=0.0, double _th=0.0)
  {
    ...
    state.pose.pose.orientation = createQuaternionFromYaw(0.0);
    ...
    gt_state.pose.pose.orientation = createQuaternionFromYaw(0.0);
  ...
  void PublishPosition()
  {
    ...
    state.pose.pose.orientation = createQuaternionFromYaw(this->th);

    gt_state.pose.pose.position.x = this->x_gt;
    gt_state.pose.pose.position.y = this->y_gt;
    gt_state.pose.pose.orientation = createQuaternionFromYaw(this->th_gt);
    {
    ...
    transform.header.stamp = ros::Time(state.header.stamp);
    transform.header.frame_id = odom_frame_id;
    transform.child_frame_id = base_frame_id;
    transform.transform.translation.x = this->x;
    transform.transform.translation.y = this->y;
    transform.transform.rotation = createQuaternionFromYaw(this->th);
    broadcaster.sendTransform(transform);
    if (pub_global_frame_) {
      geometry_msgs::TransformStamped identity_tf;
      identity_tf.header.stamp = ros::Time(state.header.stamp);
      identity_tf.header.frame_id = odom_frame_id;
      identity_tf.child_frame_id = base_frame_id;
      identity_tf.transform.rotation = createQuaternionFromYaw(0);
      broadcaster.sendTransform(identity_tf);
    }
    ...
    amcl_pose.pose.pose.orientation = createQuaternionFromYaw(this->th);
    ...
    gt_pose.pose.orientation = createQuaternionFromYaw(this->th_gt);
  ...
  void OnInitialPose(const geometry_msgs::PoseWithCovarianceStampedConstPtr &input)
  {
    ...
    this->th_gt = getYaw(input->pose.pose.orientation);
  }
};
